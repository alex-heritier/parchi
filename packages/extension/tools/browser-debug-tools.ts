...
export class BrowserDebugManager {
  ...
  async evaluate(tabId: number, expression: string, awaitPromise = true): Promise<BrowserToolResult> {
    ...
    if (response?.exceptionDetails) {
      ...
      const description =
        details.exception?.description || String(details.exception?.value || '') || details.text || 'Evaluation failed.';
      return {
        success: false,
    ...
    return {
      success: true,
      value: result?.value ?? result?.description ?? null,
      valueType: result?.type || typeof result?.value,
    };
  }
  ...
  async getNetworkLog(
    ...
    const urlIncludes = String(options.urlIncludes || '').trim().toLowerCase();
  ...
  private async sendCommand<TResult = unknown>(tabId: number, method: string, commandParams: Record<string, unknown> = {}) {
  ...
  private async finalizeRequest(tabId: number, session: DebugSession, draft: NetworkRequestDraft) {
    ...
    try {
      const response = await this.sendCommand<{ body?: string; base64Encoded?: boolean }>(tabId, 'Network.getResponseBody', {
      ...
      // Ignore body fetch failures for opaque/binary responses.
    }
...
export async function evaluateTool(ctx: BrowserToolsDelegate, args: BrowserToolArgs) {
  ...
  await ctx.sendOverlay(tabId, {
    label: 'Evaluate page JS',
    note: 'page context',
    durationMs: 900,
  });
  try {
    return await ctx.evaluateInTab(tabId, expression, args.awaitPromise !== false);
  } catch (error) {
    return {
      success: false,
      error: 'Page evaluation failed.',
...
export async function getNetworkLogTool(ctx: BrowserToolsDelegate, args: BrowserToolArgs) {
  ...
  await ctx.sendOverlay(tabId, {
    ...
    note: String(args.urlIncludes || args.method || 'recent requests'),
    durationMs: 800,
  });
