...
const makeProof = () => {
  ...
  return `PX-${Math.abs(h >>> 0).toString(16).padStart(8, '0').toUpperCase()}`;
};
const renderExport = () => {
  const done = Object.values(state.tasks).filter(Boolean).length;
...
$('requestOtp').addEventListener('click', () => {
  ...
  $('otpInbox').innerHTML = `<div>old code: 184202 (expired)</div><div>ticket: ${state.otp} (valid)</div><div>hint: use latest ticket</div>`;
  log('otp_issued');
...
$('verifyOtp').addEventListener('click', () => {
  ...
  if (ok) unlock('formCard', 'tableCard', 'kanbanCard', 'releaseCard', 'shadowCard', 'frameCard', 'scrollCard', 'richTextCard', 'assetCard');
});
$('plan').addEventListener('change', () => $('enterpriseFields').classList.toggle('hidden', $('plan').value !== 'enterprise'));
$('submitQuote').addEventListener('click', () => {
  const ok = $('region').value === 'emea'
...
const rows = [
  ['INC-1309', 'medium', 'maya', 13], ['INC-2210', 'high', 'liam', 9], ['INC-7421', 'high', 'olivia', 7],
  ['INC-8890', 'high', 'elias', 16], ['INC-5003', 'medium', 'leo', 22], ['INC-7740', 'high', 'li', 8],
];
const renderRows = () => {
  ...
  $('incidentRows').innerHTML = data.map((r) => `<tr data-ticket="${r[0]}"><td>${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td></tr>`).join('');
  [...$('incidentRows').querySelectorAll('tr')].forEach((tr) => tr.onclick = () => {
...
$('pinTicket').addEventListener('click', () => {
  const ok = state.selectedTicket === 'INC-7421'
...
[...document.querySelectorAll('.lane')].forEach((lane) => {
  lane.addEventListener('dragstart', (e) => { dragEl = e.target.closest('li'); });
  lane.addEventListener('dragover', (e) => e.preventDefault());
  lane.addEventListener('drop', (e) => { e.preventDefault(); if (dragEl) lane.appendChild(dragEl); });
});
...
class SecureToggle extends HTMLElement {
  connectedCallback() {
    ...
    root.getElementById('a').onclick = () => this.dispatchEvent(new CustomEvent('secure-apply', { detail: { mode: root.getElementById('m').value, key: root.getElementById('k').value.trim() } }));
  }
}
...
$('validateEditor').addEventListener('click', () => {
  ...
  const ok = html.includes('<h2>relay readiness</h2>')
...
$('validateAssets').addEventListener('click', () => {
  ...
  const rows = [...document.querySelectorAll('#assetRows .asset-row')].map((r) => {
    ...
    return { name: name.value.trim().toLowerCase(), tag: tag.value.trim().toLowerCase(), pri: pri.value.trim().toLowerCase() };
  });
